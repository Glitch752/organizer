...
import type { Tag } from "./htmlRender";
...
function justifyRow(row: TagObject, justify: Justification[]): Tag[] {
    ...
    return row.body.map((cell, i) => {
        ...
        if(cell.attrs.class) cell.attrs.class += ` ${alignmentClass}`;
        else cell.attrs.class = alignmentClass;
